...
import { Phone, MessageSquare, Award, CheckCircle2, Mail, ExternalLink, ShieldCheck } from 'lucide-react';
...
export const LeadershipCards: React.FC<LeadershipCardsProps> = ({ staff: propStaff, compact = false }) => {
  const { staff: contextStaff } = useSite();
  const allStaff = propStaff || contextStaff;

  // Filter central executive leadership
  const leadershipList = (allStaff && allStaff.length > 0)
    ? allStaff
  ...
  // Fallback to INITIAL_STAFF central leaders if list is empty
      ...
      {displayLeaders.map((member, index) => {
        ...
        return (
          <div
            ...
            className="group relative bg-white rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-sm hover:shadow-xl transition-all duration-300 p-6 sm:p-8 flex flex-col justify-between overflow-hidden"
          >
                ...
                <div className="relative flex-shrink-0">
                  {member.photoUrl ? (
                    <img
                      src={member.photoUrl}
                      alt={member.name}
                      className="w-18 h-18 sm:w-22 sm:h-22 rounded-2xl object-cover border-2 border-slate-200 shadow-sm group-hover:scale-102 transition-transform"
                    />
                  ) : (
                    <div
                      className={`w-18 h-18 sm:w-22 sm:h-22 rounded-2xl font-extrabold text-2xl flex items-center justify-center border-2 shadow-sm ${
                        isDirector
